			...
			bool showRectified, const string &storintrinsicsyml, const string &storextrinsicsyml)
{
    ...
    const float squareSize = 20.f;//32.f //20.f;  // Set this to your actual square size  正方形大小
    ...
    for(i = j = 0; i < nimages; i++)
    {
		...
		//imwrite("../mydata/output/test/corners2.jpg",cimg1);
...
/***************************************************************************************/
 ...
 //  if(i == 2)	imwrite("../mydata/rectified.jpg", canvas);
        char c = (char)waitKey();
        if( c == 27 || c == 'q' || c == 'Q' )
            break;
    }
   ...
   //  stereoRectify( M1, D1, M2, D2, img_size, R, T, R1, R2, P1, P2, Q, CALIB_ZERO_DISPARITY, -1, img_size, &roi1, &roi2 );
	...
	if(showRect)//showRect
	{
	  ...
	  for(int j = 0; j < canvas.rows; j += 16 )
          ...
          imshow("rectified", canvas);
	//  if(i == 0)	imwrite("../mydata/rectified.jpg", canvas);
          char c = (char)waitKey();
          if( c == 27 || c == 'q' || c == 'Q' )
            break;
	}
