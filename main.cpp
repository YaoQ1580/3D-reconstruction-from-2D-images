...
#define PHASE_THRESHOLD  0.12 //model40 model62 0.05 // model3 0.2  // model20 0.01(singlematch) model1 0.01
...
//     double sf;
//     int w, h;
    Mat img1 = imread("../myimages/Model9/Rect/L/Graycodeleft101.bmp",0);
    Mat img2 = imread("../myimages/Model9/Rect/R/Graycoderight101.bmp",0);

//     f = 600./MAX(img1.cols, img1.rows);  //600
//     w = cvRound(img1.cols*sf);
//     h = cvRound(img1.rows*sf);
    canvas.create(img1.rows, img1.cols*2, CV_8UC3);
    
    Mat cimg1,cimg2;
    cvtColor(img1, cimg1, COLOR_GRAY2BGR);
    cvtColor(img2, cimg2, COLOR_GRAY2BGR);
    
    Mat canvasPart = canvas(Rect(0, 0, img1.cols, img1.rows));
    resize(cimg1, canvasPart, canvasPart.size(), 0, 0, INTER_AREA);
    
    canvasPart = canvas(Rect(img1.cols, 0, img1.cols, img1.rows));
    resize(cimg2, canvasPart, canvasPart.size(), 0, 0, INTER_AREA);
    
    for(int k = 0; k < leftfeaturepoints.size(); k++)
    {
      Point2i point1, point2;
      point1.x = leftfeaturepoints[k].x;
      point1.y = leftfeaturepoints[k].y;
      point2.x = rightfeaturepoints[k].x + img1.cols;
      point2.y = rightfeaturepoints[k].y;
      
      circle(canvas, point1, 2, Scalar(0, 0, 255));
      circle(canvas, point2, 2, Scalar(0, 0, 255));
      
    //  line(canvas, point1, point2, Scalar(0, 0, 255));
    }
    
    
    imwrite("../mydata/output/canvass.jpg",canvas);
#endif
