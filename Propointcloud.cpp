...
const char* pnts3D_filtered_pcdfilename = "../mydata/output/pnts3D_pcd_filteredmm1.pcd";
...
void savepntsPCD(cv::Mat& pnts)
{
  ...
  // filterpnts3D.at<float>(i/2000, i%2000) = pnts3D_data3/1000.0f;
    ...
    {
      cloud.points[i].x = pnts3D_data1;
      cloud.points[i].y = pnts3D_data2;
      cloud.points[i].z = pnts3D_data3;  //mm covert to m
...
void filterpointcloud(void)
{
 ...
 // reader.read("../mydata/output/result_pcd45.pcd", *cloud);
  ...
  sor.setLeafSize(1, 1, 1); // model40:0.002f, 0.002f, 0.003f; 0.0002f, 0.0002f, 0.0005f
  ...
  std::cout << "Pointcloud after filtering: " << cloud_filtered->width*cloud_filtered->height << "data points" <<endl;
   ...
   pass.setFilterLimits(1900, 2200);// model21 pass.setFilterLimits(1.45,1.9);
  ...
  outrem.setRadiusSearch(0.01);   //model71  0.012  0.01
  outrem.setMinNeighborsInRadius(60);//30
  ...
  /*************************save filtered pnts3D******************************/
  pcl::PCDWriter writer;
  writer.write(pnts3D_filtered_pcdfilename, *cloud_res);
 ...
 /***********************Surface Reconstruction****************************************/
  ...
  gp3.setSearchRadius(15);  //0.025
  
  gp3.setMu(3.0);   //2.5
  gp3.setMaximumNearestNeighbors(100);
  gp3.setMinimumAngle(CV_PI/18);
  gp3.setMaximumAngle(3*CV_PI/4);
  gp3.setMaximumSurfaceAngle(CV_PI/4);   //cv_pi/2
...
void poissonreconstruction(void)
{
  ...
  pcl::PCDReader reader;
  reader.read(pnts3D_pcdfilename, *cloud);
 // reader.read("../mydata/output/result_pcd123.pcd", *cloud);
...
/**********************mls*****************************************************/
  ...
  pass.setFilterFieldName("z");
   pass.setFilterLimits(1.8,2.2);// model21 pass.setFilterLimits(1.45,1.9);
  pass.setFilterLimitsNegative(false);
...
void viewer_ply(void)
{
  ...
  reader.read("../mydata/input/cloud/3_HDR.ply",clod);
  pcl::PCDWriter writer;
  writer.writeASCII("../mydata/input/cloud/3_HDR.pcd", clod);
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZRGB>);
  
  if (pcl::io::loadPLYFile<pcl::PointXYZRGB>("../mydata/input/cloud/3_HDR.pcd", *cloud) == -1) //* load the file 
